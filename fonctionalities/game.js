...
var loot_table = ["rien","une armure","une épée","une bourse"];
...
function initialize() {
	...
	document.getElementById('LogAction').innerHTML = "Le maire vous présente :  <br>";
	document.getElementById('LogAction').innerHTML +="***DRAGON ***"+game.DragonName+" "+game.DragonTitle+"***  : "+game.DragonHP+" PV <br> ***LE BRAVE***  : "+game.PlayerName+"*** : "+game.KnightHP+" PV <br>";
	document.getElementById("LogAction").innerHTML += "<br><b>Appuyer sur le bouton pour continuer</b>"
...
function PlayOneRound ()
{
	...
	if (knightSpeed>monsterSpeed) 	{
		...
		document.getElementById('LogAction').innerHTML +="Vous avez été plus rapide que l'ennemi ! Vous le frappez et lui enlevez "+ptsGagnes+"PV !<br>";
	}
	else 	{
		if (game.monstreEnCours!=DRAGON) {
			ptsPerdus = MonsterDamage();
			game.KnightHP=game.KnightHP-ptsPerdus;
			document.getElementById('LogAction').innerHTML +="Vous avez été moins rapide que l'ennemi ! Il vous frappe et vous enlève "+ptsPerdus+"PV !<br>"
		}
		else {
			ptsPerdus = MonsterDamage();
			game.KnightHP=game.KnightHP-ptsPerdus;
...
function PlaydragonAllRounds() {
	...
	while (game.KnightHP>0 && game.DragonHP>0)  	{
		...
		document.getElementById('LogAction').innerHTML ="<br>===Tour n°"+game.round+"===<br>";
		document.getElementById('LogAction').innerHTML +="***"+game.DragonName+" "+game.DragonTitle+"*** : "+game.DragonHP+" PV <br> ***"+game.PlayerName+"*** : "+game.KnightHP+" PV <br>";
		document.getElementById("LogAction").innerHTML += "<br><b>Appuyer sur le bouton pour continuer</b>"
		game.round++;
	}
}
...
function PlayMonsterAllRounds() {
	...
	if (monsterRound > 1) {
		document.getElementById('LogAction').innerHTML +="=== Après "+monsterRound+" échange de coups... ===<br>";
	}
	else {
		document.getElementById('LogAction').innerHTML +="=== Après "+monsterRound+" échanges de coups... ===<br>";
	}
	document.getElementById('KHP').textContent = game.KnightHP;
	document.getElementById('Gold').textContent = game.gold;
...
function checkGold(mise)  {
	...
	if (game.gold<mise)  {
		document.getElementById('LogAction').innerHTML = '"Pas assez de pièces ? Partez! " <br>';
		coins = 0
	}
	else {
		coins = mise;
		...
		if (game.destination==2)  {
			...
			switch (game.encounter)  {
				case MONSTER: 				{
					...
					document.getElementById('LogAction').innerHTML +="Vous avez terrassé un monstre qui trainaît aux alentours, récupérant ainsi "+loot_table[game.loot]+". Cela va sûrement intéresser le marchand en ville. <br>Allez le voir avant votre prochain combat <br>";
				}
				break;
			case CHEST :  	{
				coins=getRandomInteger(5,100);
				game.gold+=coins;
				document.getElementById('LogAction').innerHTML +="Vous avez trouvé "+coins+" pièces dans un coffre caché, augmentant ainsi votre total à "+game.gold+" pièces.<br>";
				break;
			}
			case CORPSE : 	{
				...
				document.getElementById('LogAction').innerHTML +="Vous avez trouvé "+coins+" pièces sur un cadavre, augmentant ainsi votre total à "+game.gold+" pièces. <br>Il y avait aussi un sac scellé sur lui, le marchand en ville doit pouvoir desceller ça.<br>Conseil : allez le voir avant de risquer de le perdre dans un combat.<br>";
				break;
			}
			case CHASSEUR_MONOLOGUE : 	{
				//console.log ("allié ...");
				showImage("images/chevalier_perdu.jpg");
				document.getElementById('LogAction').innerHTML +='Vous rencontrez un autre chasseur de dragon qui vous dit : "Bonjour, mercenaire ! <br> ';
				...
				switch (hint) {
					case 1 : 	{
						document.getElementById('LogAction').innerHTML +="Saviez-vous que les morts-vivants ont tendance à ne s'approcher que de ceux n'ayant pas meilleure armure qu'une armure en cuivre ?<br>";
						break;
					}
					case 2 :	{
						document.getElementById('LogAction').innerHTML +="Je me demande pourquoi excepté nous humains, on ne rencontre que des morts-vivants ou des ogres dans cette forêt...<br>";
						break;
					}
					case 3 :	{
						document.getElementById('LogAction').innerHTML +="Est-ce qu'il y a quelqu'un qui sait vraiment ce qui se cache derrière le 4ème mur de la ville ? Des rumeurs disent qu'il n'y a que des lignes et des lignes de texte incompréhensibles...<br>";
						break;
					}
					case 4 :	{
						document.getElementById('LogAction').innerHTML +="Il paraît que le fondateur de la ville a tenté de récupérer la princesse via la magie, mais que celle-ci l'a trahi, le transformant ainsi que toutes ses futures victimes en morts-vivants.<br>";
						break;
					}
					case 5 :	{
						document.getElementById('LogAction').innerHTML +="Saviez-vous que vous ne pouvez mourir d'âge ici ? Le soleil est immobile dans le ciel, il n'y a jamais une once de vent et la météo reste la même tout le temps. Une autre conséquence de la magie du fondateur.<br>";
						break;
					}
				}
				document.getElementById('LogAction').innerHTML +='<br>Peu importe ce qui ce passe, ce qui est important est que tu ne meures pas de saignement, donc va te soigner à la ville à chaque fois." <br>';
			}
		}
		...
		case 1 : 	{
			...
			if (reponse=="oui") 	{
				...
				if (coins !=0) 	{
					...
					document.getElementById('KHP').innerHTML = game.KnightHP;
					document.getElementById('LogAction').innerHTML +="Vous êtes passé à l'église vous faire soigner. Vos points de vie remontent.<br>";
				}
				else {
					document.getElementById('LogAction').innerHTML +="Vous êtes passé à l'église mais vous n'avez pas assez de pièces pour vous faire soigner.<br>";
				}
			}
			else {
				document.getElementById('LogAction').innerHTML +="Passer à l'église sans vous faire soigner ! Quel dommage ...<br>";
			}
			break;
		}
		case 2 : 	{
			...
			if (reponse=="oui") 	{
				...
				if (coins !=0) 	{
					...
					document.getElementById('KHP').innerHTML = game.KnightHP;
					document.getElementById('LogAction').innerHTML +="Vous êtes passé à l'auberge vous reposer. Vos points de vie remontent.<br>";
				}
				else {
					document.getElementById('LogAction').innerHTML +="Vous êtes passé à l'auberge vous reposer, mais sans argent on vous refuse.<br>";
				}
			}
			else {
				document.getElementById('LogAction').innerHTML +="Vous êtes passé à l'auberge sans vous arrêter.<br>";
			}
			break;
		}
		case 3 : 	{
				...
				if (reponse==1) 	{
					...
					if (reponse == 1) 	{
						...
						if (coins !=0) 	{
							game.gold -= coins;
							game.sword=2;
							document.getElementById('LogAction').innerHTML +="Vous avez maintenant une épée en acier<br>";
						}
						else  {
							document.getElementById('LogAction').innerHTML +="Pas assez d'argent ... <br>";
						}
					}
					else if (reponse == 2) 	{
						...
						if (coins !=0) 	{
							game.gold -= coins;
							game.sword=3;
							document.getElementById('LogAction').innerHTML +="Vous avez maintenant l'épée légendaire<br>";
						}
						else 	{
							document.getElementById('LogAction').innerHTML +="Pas assez d'argent ... <br>";
						}
					...
					if (reponse == 1) 	{
						...
						if (coins !=0) 	{
							game.gold -= coins;
							game.armor=2;
							document.getElementById('LogAction').innerHTML +="Vous avez maintenant une armure en fer.<br>";
						}
						else 	{
							document.getElementById('LogAction').innerHTML +="Pas assez d'argent ... <br>";
						}
						...
						if (coins !=0) 	{
							game.gold -= coins;
							game.armor=3;
							document.getElementById('LogAction').innerHTML +="Vous avez maintenant l'amure magique !!! <br>";
						}
						else 	{
							document.getElementById('LogAction').innerHTML +="Pas assez d'argent ... <br>";
						}
			...
			document.getElementById('LogAction').innerHTML +="Vous êtes passé chez le forgeron pour regarder, sans plus ...<br>";
			break;
		}
		case 4 : 	{
			...
			// Excepté pour le massacre, le marchand est le seul moyen de gagner de l'argent. Les prix sont à 25% de ce que le forgeron demande pour le niveau 2 d'équipement, sauf la bourse scellée qui ne contient que peu de base.
			...
			if (reponse=="oui") 	{
				switch (game.loot) 	{
					case LOOT_ARMOR : 	{
						...
						if (reponse=="oui") 	{
							game.gold += 75;
							game.loot = 0;
							document.getElementById('LogAction').innerHTML +="Vous avez vendu l'armure pour 75 pièces seulement. Pas mal pour un objet qui ne vous a rien coûté ...<br>";
						}
						break;
					}
					case LOOT_SWORD : 	{
						...
						if (reponse=="oui") 	{
							game.gold += 25;
							game.loot = 0;
							document.getElementById('LogAction').innerHTML +="Vous avez vendu l'épée pour 25 pièces seulement. Elle devait être rouillée...<br>";
						}
						break;
					}
					case LOOT_PURSE : 	{
						...
						if (reponse=="oui") 	{
							game.gold += 15;
							game.loot = 0;
							document.getElementById('LogAction').innerHTML +="Vous avez gagné 15 pièces seulement. Belle affaire !<br>";
						}
						break;
					}
					default : 	{
						document.getElementById('LogAction').innerHTML +="Marchand : Je suis désolé, mais vous n'avez rien sur vous qui retient mon attention.<br><br>"
					}
				}
			}
			else {
				...
				if (reponse == "oui") 	{
					...
					if (coins !=0) 	{
						game.gold -= coins;
						document.getElementById('LogAction').innerHTML = "<i>Vous avez l'impression de vous être fait arnaquer... Mais cette plume rejoindra votre collection ne serait-ce que par sa beauté. Si vous restez en vie...<i><br>";
					}
				}
				else 	{
					document.getElementById('LogAction').innerHTML +="Vous avez refusé l'achat d'une plume de phénix.<br>"
				}
			}
		}
	}
...
function jouer() {
	...
	if (game.KnightHP>0 && game.DragonHP<=0) {
		...
		alert("<br>Vous avez terrassé le terrible ***"+game.DragonName+" "+game.DragonTitle+"*** et délivré la princesse ! Mais pas sans séquelles de votre dernier combat...");
		...
		alert("<br>***"+game.DragonName+" "+game.DragonTitle+"*** a gagné, vous avez été terrassé(e) ! La princesse restera sa captive pour les 100 ans à venir.");
